...
public class ThirdPersonCameraNET : MonoBehaviour
{
	...
	// If target and/or camera is not set, try using fallbacks
	{
		...
		if (camera == null)
		{
			...
			{
				camera = Camera.main;
                //camera.transform.position = camera.transform.position + camera.transform.right * 0.2f + camera.transform.up * 0.2f;
	...
	// See if the camera touches the ground and adjust the camera distance if an object blocks the view
	{
		...
		// Cast a sphere from the target towards the camera - using the view radius - checking against the obstacle layers
			targetDistance = Mathf.Min ((hit.point - target.transform.position).magnitude, optimalDistance); // If something is hit, set the target distance to the hit position
		else
			targetDistance = optimalDistance; // If nothing is hit, target the optimal distance
	}
	...
	{
        optimalDistance = camDistance;
        //optimalDistance = Mathf.Clamp ( optimalDistance + Input.GetAxis ("Mouse ScrollWheel") * -zoomSpeed * Time.deltaTime,minDistance,maxDistance);
    }
	...
	// Update camera position - specifics are delegated to camera mode functions
	{
		...
		{
			if (controlLock) Screen.lockCursor = true;
		...
		{
			if (controlLock) Screen.lockCursor = false;
			
			Vector3 movement = target.transform.position - lastStationaryPosition;
			if (new Vector2 (movement.x, movement.z).magnitude > movementThreshold) FollowUpdate ();
		}
	...
	// Apply any change in camera distance
	{
		Vector3 targetPosition = target.transform.position
            + camera.transform.up * camOffsetY + camera.transform.right * camOffsetX
            + (camera.transform.position 
            - (target.transform.position + camera.transform.up * camOffsetY + camera.transform.right * camOffsetX)).normalized * targetDistance
